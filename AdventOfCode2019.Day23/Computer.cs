...
namespace AdventOfCode2019.Day23
{
    public class Computer
    {
        ...
        public Computer(IEnumerable<long> program, int address)
        {
            _program = program
                .Select((v, i) => new {v, i})
                .ToDictionary(p => (long) p.i, p => p.v);
            Input = new Queue<long>(new[] {(long) address});
        ...
        {
            Idle = !Input.TryDequeue(out var value);
            if (Idle)
            {
                value = -1;
            }

            SetValue(_program[_pointer + 1], mode, value);
            _pointer += 2;
        }

        private void Write(long mode1)
        {
            Output.Enqueue(GetValue(_program[_pointer + 1], mode1));
            _pointer += 2;
        }
